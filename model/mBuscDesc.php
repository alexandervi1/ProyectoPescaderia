...
if (isset($_GET["id"])) {
    ...
    // Consulta para obtener el descuento del producto
    $sql = "SELECT descuento FROM Producto WHERE producto_id = $RecogeOpcion";
    $result = mysqli_query($conn, $sql);

    if ($result && mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $descuentoProducto = trim($row["descuento"]); // Descuento del producto

        ob_end_clean();
        echo $descuentoProducto; // Devolver el descuento del producto
        exit;
    } else {
        ob_end_clean();
