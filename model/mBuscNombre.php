...
if (isset($_GET["id"])) {
    ...
    $sql = "SELECT nombre FROM Producto WHERE producto_id = $RecogeOpcion";
    $result = mysqli_query($conn, $sql);

    if ($result && mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $nombreProducto = trim($row["nombre"]); // Nombre del producto

        ob_end_clean();
        echo $nombreProducto; // Devolver el nombre del producto
        exit;
    } else {
        ob_end_clean();
