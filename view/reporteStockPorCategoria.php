...
// ====================================================================
// 2. FUNCIÓN PARA OBTENER DATOS DEL REPORTE DE STOCK ACTUAL POR CATEGORÍA
// ====================================================================
function getReporteStockPorCategoria($pdo) {
    $sql = "
        SELECT
    ...
    try {
        ...
        error_log("Error al ejecutar la consulta de reporte de stock por categoría: " . $e->getMessage());
        die("Lo sentimos, ocurrió un error al obtener los datos del reporte. Por favor, inténtelo más tarde.");
    }
}

$stockData = getReporteStockPorCategoria($pdo);

// ====================================================================
// 3. CÁLCULO DE RESÚMENES
// ====================================================================
$totalProductosDistintos = count($stockData);
$totalUnidadesEnStock = 0;
foreach ($stockData as $item) {
    $totalUnidadesEnStock += (float)$item['stockActual'];
}

// La etiqueta para el total general de unidades se mantiene flexible.
$totalUnidadesLabel = "Unidades Totales en Stock"; 
if (!empty($stockData)) {
    $firstUnit = $stockData[0]['unidadCompra'] ?? '';
    if (strtolower($firstUnit) === 'kg' || strtolower($firstUnit) === 'kgs') {
        $totalUnidadesLabel = "Total en Kilogramos";
    } elseif (strtolower($firstUnit) === 'ton' || strtolower($firstUnit) === 'toneladas' || strtolower($firstUnit) === 'tn') {
        $totalUnidadesLabel = "Total en Toneladas";
    } elseif (strtolower($firstUnit) === 'lb' || strtolower($firstUnit) === 'libras') {
        $totalUnidadesLabel = "Total en Libras";
    }
}
...
<head>
    ...
    <title>Reporte de Stock por Categoría - Pescadería Don Walter</title>
...
<body>
    ...
    <main class="container report-section">
        <h1 class="mb-4">Reporte de Stock Actual por Categoría</h1>
        <p class="lead text-muted mb-5">
            Inventario detallado de los productos disponibles, agrupados por su categoría y mostrando la cantidad en la unidad de compra correspondiente.
        </p>
                ...
                <div class="row text-center">
                    <div class="col-md-6">
                        <div class="p-3 border rounded">
                            <h4 class="text-info"><?php echo $totalProductosDistintos; ?></h4>
                            <p class="text-muted mb-0">Productos Distintos en Stock</p>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="p-3 border rounded">
                            <h4 class="text-success"><?php echo number_format($totalUnidadesEnStock, 2); ?></h4>
                            <p class="text-muted mb-0"><?php echo $totalUnidadesLabel; ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover border">
                ...
                <tbody>
                    <?php
                    if (empty($stockData)) {
                        echo '<tr><td colspan="4" class="text-center text-muted py-4">No hay datos de stock para mostrar.</td></tr>';
                    } else {
                        ...
                        foreach ($stockData as $item) {
                            ...
                            if ($item['categoria'] !== $currentCategory) {
                                $rowspanCount = 0;
                                foreach ($stockData as $subItem) {
                                    if ($subItem['categoria'] === $item['categoria']) {
                                        $rowspanCount++;
                                    }
                                }
                                echo '<td rowspan="' . $rowspanCount . '"><strong>' . htmlspecialchars($item['categoria']) . '</strong></td>';
