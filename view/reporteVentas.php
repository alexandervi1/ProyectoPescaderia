...
// ====================================================================
// 2. FUNCIÓN PARA OBTENER DATOS DEL REPORTE
// ====================================================================
function getReporteVentasDetallado($pdo) {
    $sql = "
        SELECT
            ...
            (pp.cantidad * prod.precio) AS subtotalProducto
        FROM
            pedido p
        JOIN
            usuario u ON p.usuario_id = u.usuario_id
        JOIN
            pedidoproducto pp ON p.pedido_id = pp.pedido_id
        JOIN
            producto prod ON pp.producto_id = prod.producto_id
        ORDER BY
            p.fecha_pedido DESC, p.pedido_id ASC, prod.nombre ASC;
    ";
    try {
        ...
        error_log("Error al ejecutar la consulta de reporte de ventas: " . $e->getMessage());
        die("Lo sentimos, ocurrió un error al obtener los datos del reporte. Por favor, inténtelo más tarde.");
    }
}

$ventasData = getReporteVentasDetallado($pdo);

// ====================================================================
// 3. CÁLCULO DE RESÚMENES
// ====================================================================
$totalPedidosUnicos = [];
$totalIngresos = 0;
...
foreach ($ventasData as $venta) {
    $totalPedidosUnicos[$venta['idPedido']] = true;
    $totalIngresos += (float)$venta['subtotalProducto'];
    $totalProductosVendidos += (float)$venta['cantidad'];
}

$countTotalPedidos = count($totalPedidosUnicos);
...
<head>
    ...
    <title>Reporte de Ventas - Pescadería Don Walter</title>
...
<body>
    ...
    <main class="container report-section">
        <h1 class="mb-4">Reporte de Ventas Detallado</h1>
        <p class="lead text-muted mb-5">Visión completa de cada transacción y los productos vendidos por Pescadería Don Walter.</p>

        <div class="card shadow-sm mb-5">
            <div class="card-header">
                <h5 class="mb-0">Resumen General de Ventas</h5>
            </div>
            <div class="card-body">
                <div class="row text-center">
                    <div class="col-md-4">
                        <div class="p-3 border rounded">
                            <h4 class="text-info"><?php echo $countTotalPedidos; ?></h4>
                            <p class="text-muted mb-0">Total de Pedidos</p>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="p-3 border rounded">
                            <h4 class="text-success">$<?php echo number_format($totalIngresos, 2); ?></h4>
                            <p class="text-muted mb-0">Total de Ingresos</p>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="p-3 border rounded">
                            <h4 class="text-warning"><?php echo number_format($totalProductosVendidos, 2); ?></h4>
                            <p class="text-muted mb-0">Total de Productos Vendidos</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover border">
                ...
                <tbody>
                    <?php
                    if (empty($ventasData)) {
                        echo '<tr><td colspan="8" class="text-center text-muted py-4">No hay datos de ventas para mostrar.</td></tr>';
                    } else {
                        $currentPedidoId = null;
                        $rowSpanCounts = [];

                        foreach ($ventasData as $venta) {
                            if (!isset($rowSpanCounts[$venta['idPedido']])) {
                                $rowSpanCounts[$venta['idPedido']] = 0;
                            }
                            $rowSpanCounts[$venta['idPedido']]++;
                        }

                        foreach ($ventasData as $index => $venta) {
                            echo '<tr>';
                            if ($venta['idPedido'] !== $currentPedidoId) {
                                $rowspan = $rowSpanCounts[$venta['idPedido']];
                                echo '<td rowspan="' . $rowspan . '">' . htmlspecialchars($venta['idPedido']) . '</td>';
                                echo '<td rowspan="' . $rowspan . '">' . htmlspecialchars($venta['fecha']) . '</td>';
                                echo '<td rowspan="' . $rowspan . '">' . htmlspecialchars($venta['cliente']) . '</td>';
                                echo '<td rowspan="' . $rowspan . '">$' . number_format((float)$venta['totalPedido'], 2) . '</td>';
                                $currentPedidoId = $venta['idPedido'];
                            }
                            echo '<td>' . htmlspecialchars($venta['producto']) . '</td>';
                            echo '<td>' . number_format((float)$venta['cantidad'], 2) . '</td>';
                            echo '<td>$' . number_format((float)$venta['precioUnitario'], 2) . '</td>';
                            echo '<td>$' . number_format((float)$venta['subtotalProducto'], 2) . '</td>';
                            echo '</tr>';
                        }
                    }
                    ?>
