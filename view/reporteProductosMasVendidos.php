...
// ====================================================================
// 2. FUNCIÓN PARA OBTENER DATOS DEL REPORTE DE PRODUCTOS MÁS VENDIDOS
// ====================================================================
function getReporteProductosMasVendidos($pdo) {
    $sql = "
        SELECT
            p.nombre AS producto,
            c.nombre AS categoria,
            SUM(pp.cantidad) AS totalCantidadVendida,
            SUM(pp.cantidad * p.precio) AS totalIngresoProducto
        FROM
            pedidoproducto pp
        JOIN
        ...
        GROUP BY
            p.producto_id, p.nombre, c.nombre
        ORDER BY
            totalCantidadVendida DESC, totalIngresoProducto DESC;
    ";
    try {
        ...
        error_log("Error al ejecutar la consulta de reporte de productos más vendidos: " . $e->getMessage());
        die("Lo sentimos, ocurrió un error al obtener los datos del reporte. Por favor, inténtelo más tarde.");
    }
}

$productosMasVendidos = getReporteProductosMasVendidos($pdo);

// ====================================================================
// 3. CÁLCULO DE RESÚMENES PARA PRODUCTOS MÁS VENDIDOS
// ====================================================================
$cantidadTotalDeProductosDistintosVendidos = count($productosMasVendidos);
$totalGeneralCantidadesVendidas = 0;
$totalGeneralIngresosPorProductos = 0;

foreach ($productosMasVendidos as $item) {
    $totalGeneralCantidadesVendidas += (float)$item['totalCantidadVendida'];
    $totalGeneralIngresosPorProductos += (float)$item['totalIngresoProducto'];
}
...
<head>
    ...
    <title>Reporte de Productos Más Vendidos - Pescadería Don Walter</title>
...
<body>
    ...
    <main class="container report-section">
        ...
        <p class="lead text-muted mb-5">
            Este reporte muestra un ranking de los productos más vendidos por cantidad y por ingresos.
        </p>

        <div class="card shadow-sm mb-5">
            <div class="card-header">
                <h5 class="mb-0">Resumen de Productos Vendidos</h5>
            </div>
            <div class="card-body">
                <div class="row text-center">
                    <div class="col-md-4">
                        <div class="p-3 border rounded">
                            <h4 class="text-info"><?php echo $cantidadTotalDeProductosDistintosVendidos; ?></h4>
                            <p class="text-muted mb-0">Productos Distintos Vendidos</p>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="p-3 border rounded">
                            <h4 class="text-success"><?php echo number_format($totalGeneralCantidadesVendidas, 2); ?></h4>
                            <p class="text-muted mb-0">Unidades Vendidas (Total)</p>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="p-3 border rounded">
                            <h4 class="text-warning">$<?php echo number_format($totalGeneralIngresosPorProductos, 2); ?></h4>
                            <p class="text-muted mb-0">Ingresos Generados por Productos</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-hover border">
                ...
                <tbody>
                    ...
                    if (empty($productosMasVendidos)) {
                        echo '<tr><td colspan="5" class="text-center text-muted py-4">No hay datos de productos vendidos para mostrar.</td></tr>';
                    } else {
                        $rank = 1;
                        foreach ($productosMasVendidos as $producto) {
                            ...
                            echo '<td>' . number_format((float)$producto['totalCantidadVendida'], 2) . '</td>';
                            echo '<td>$' . number_format((float)$producto['totalIngresoProducto'], 2) . '</td>';
                            echo '</tr>';
                        }
                    }
                    ?>
